...
```bash
uv pip compile --extra=test --extra=docs --extra=map --extra=gef --extra=notebook --output-file=requirements.txt pyproject.toml
```

To update the requirements within the defined ranges, run:

```bash
uv pip compile --upgrade --extra=test --extra=docs --extra=map --extra=gef --extra=notebook --output-file=requirements.txt pyproject.toml
```